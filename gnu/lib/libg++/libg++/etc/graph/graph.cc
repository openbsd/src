  ...
  if (grid_style)
    {
      ...
      double x_tick, x_tick_value;

      if (x_log_scale)
	{
	  double logmin = log10 (xmin);
	  double logmax = log10 (xmax);
	  x_tick = tick_interval (no_of_ticks, logmin, logmax);
	  x_tick_value =
	    pow (10., x_tick * (x_tick > 0.
				? ceil (logmin * A_HAIR_MORE / x_tick)
				: floor  (logmin * A_HAIR_MORE / x_tick)));
	}
      else
	{
	  x_tick = tick_interval (no_of_ticks, xmin, xmax);
	  x_tick_value = x_tick
