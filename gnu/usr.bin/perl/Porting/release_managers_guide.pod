...
The Perl core ships with so-called "dual-life" modules, which are also
available via CPAN. Some of them are "core-first", others are "CPAN-first".
The goal of the synchronisation step is to ensure they don't drift too
far apart from each other between their respective releases.

Synchronisation can happen at any point during the development phase
(except feature freeze).

=head4 Find which modules are currently not synchronised
...
=head4 Sync CPAN modules with the corresponding F<cpan/> distributions

Before synchronising a distribution, you should check if a Pull Request
exists for it.  If such a PR exists, the synchronisation will happen when
the PR is merged.  If it's tagged as C<defer-next-dev> (meaning next dev
I<cycle>), do not merge the request or synchronise the module manually.

In most cases, once a new version of a distribution shipped with core has been
uploaded to CPAN, the core version thereof can be synchronised automatically
with the program F<Porting/sync-with-cpan>. For example:

 $ perl Porting/sync-with-cpan Archive::Tar

Note that for blead-first dual-life modules, the F<Porting/sync-with-cpan>
script will need the C<--type dist> option, to look for the module in
...
C<git checkout>, if necessary. (Customization is supposed to be temporary
until upstream accepts our patches, so check whether it is still needed.)
Add any customizations that may newly be needed now. Also,
restore any files that are mentioned in C<@IGNORE>, but were checked
into the repository anyway.

For example, F<version.pm> is a case of C<CUSTOMIZED> distribution
where the blead version of the file is preferred over the CPAN version.
This means that F<cpan/version/lib/version.pm> is not modified by the
sync, and you'll have to apply the relevant changes manually.
(Though F<version.pm> is special in that this will I<always> be the case,
...
B<Any problems listed as pedantic aren't worth your time investigating.>
...
=for checklist skip MAINT BLEAD-POINT

=head3 Merge BLEAD-POINT perldeltas

When doing the first RC for BLEAD-FINAL, all perldeltas from the 5.X series
must be merged into F<pod/perldelta.pod>. The process starts with
...
Be sure you are familiar with F<Porting/how_to_write_a_perldelta.pod>.

The pull request template encourages contributors to include perldelta
updates when appropriate, thus there tends to be few updates left to the
release manager to write.
...
relating to their recent work.

To identify possible remaining work, it is useful to review merged PRs:
L<https://github.com/perl/perl5/pulls>. It may be useful to add the
parameters merged requests since last release, for example:
C<< is:pr is:merged merged:>2026-03-20 >>

With this, you have enough to decide if the perldelta content is in a
satisfactory state.  Make sure that every section is, if necessary,
populated and complete.  Keep in mind that some mostly mechanical steps
will happen later, for example, removing the C<XXX> entries and updating
acknowledgements.  You won't be able to automatically fill in the
"Updated Modules" section until after L<Module::CoreList> is updated (as
described below in L</"Update Module::CoreList">).
...
Be sure to commit your changes:
 ...
 $ git commit -m 'Add 5.X.Y to perlhist' pod/perlhist.pod
...
Submit a pull request to L<https://github.com/perlorg/perlweb> to
update the link in F<docs/shared/tpl/stats.html>.
