...
ffeexpr_cb_close_paren_ (ffelexToken ft, ffebld expr, ffelexToken t)
{
  ...
  /* First push the (parenthesized) expression as an operand onto the
     expression stack. */

  e = ffeexpr_expr_new_ ();
  e->type = FFEEXPR_exprtypeOPERAND_;
  e->u.operand = ffebld_new_paren (expr);
  ffebld_set_info (e->u.operand, ffeinfo_use (ffebld_info (expr)));
...
ffeexpr_token_name_rhs_ (ffelexToken t)
{
  ...
  switch (ffelex_token_type (t))
	...
	case FFEEXPR_parentypeSUBROUTINE_:	/* Invalid case. */
	case FFEEXPR_parentypeANY_:
	  ffebld_set_info (e->u.operand, ffesymbol_info (s));
	  return
	    (ffelexHandler)
